...
This is why objecting that LLMs don't understand what they write (and there is an entire field of PhDs implied before we even pass over the question of whether they do actually write, and how, and why not) is not a critique of LLMs with any content other than class ressentiment. LLMs have been structured and instructed (novel PhD field 2) with a fitness function of _being compelling_. In some cases the truth is compelling. In others, it isn't what you want to hear. Where the truth is not satisfying or available but its aesthetics are required, LLMs will do their best to keep their interlocutors happy (we are up to at least five PhD departments here). If this requires producing an answer that references a point in the LLM's latent space that does not simply map onto a pre-existing fact.
